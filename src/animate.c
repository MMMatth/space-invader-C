...
void init_animates(char * folder_path, int nb_frame, Uint32 frame_duration, animate_t ** animate){
    for (int i = 0; i < MAX_ANIM; i++){
        animate[i] = malloc(sizeof(struct animate_s));
        init_animate(folder_path, nb_frame, frame_duration, animate[i]);
    }
}
...
void apply_animate(SDL_Renderer * renderer ,animate_t * animate, SDL_Texture ** tab_texture, int x, int y, int w, int h){
    ...
    if (animate->current_time == animate->nb_frames - 1) animate->active = 0; // si on est au dernier frame on desactive l'animation
